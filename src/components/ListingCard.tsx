...
export function ListingCard({ listing }: ListingCardProps) {
  ...
  return (
    <div onClick={() => navigate(`/listing/${listing.id}`)} className="bg-card rounded-xl border border-border overflow-hidden shadow-card animate-scale-in hover:shadow-lg transition-shadow cursor-pointer">
      <div className="relative aspect-[4/3] md:aspect-[3/2] bg-muted">
        ...
        <Badge className={`absolute top-3 left-3 ${listing.type === "donate" ? "bg-success text-success-foreground" : "bg-primary text-primary-foreground"}`}>
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-start justify-between gap-2">
          ...
          {listing.type === "sell" && listing.price ? (
            ...
            <p className="text-lg font-bold text-success shrink-0">{t("listing.free")}</p>
          )}
        </div>
          ...
          <div className="flex items-center gap-2">
            {listing.profiles?.avatar_url ? (
              ...
              <div className="w-6 h-6 rounded-full bg-accent flex items-center justify-center">
                <span className="text-[10px] font-bold text-accent-foreground">{(listing.profiles?.display_name || "?")[0].toUpperCase()}</span>
              </div>
            )}
            <span className="text-xs text-muted-foreground">{listing.profiles?.display_name || t("listing.anonymous")}</span>
          </div>
          <span className="text-xs text-muted-foreground">{formatDistanceToNow(new Date(listing.created_at), { addSuffix: true, locale: dateLocale })}</span>
        </div>
      </div>
    </div>
  );
