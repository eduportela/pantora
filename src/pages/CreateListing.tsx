...
export default function CreateListing() {
      ...
      supabase
        ...
        .then(({ data }) => {
          if (data) {
            ...
            setBottleCount(data.bottle_count?.toString() || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim()) { toast.error("Legg til en tittel"); return; }
    if (!location.trim()) { toast.error("Legg til en lokasjon"); return; }
    ...
    try {
      ...
      const listingData = {
        ...
        type: (!price || parseInt(price) === 0) ? "donate" : "sell",
        images: imageUrls,
        user_id: user.id,
      };
    ...
    <div className="min-h-screen bg-background pb-8">
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-border safe-area-pt">
        <div className="flex items-center gap-3 px-4 py-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-foreground">{editId ? "Rediger annonse" : "Ny annonse"}</h1>
            <p className="text-sm text-muted-foreground">
              {editId ? "Oppdater annonsen din" : "Legg ut flasker for salg eller donasjon"}
            </p>
          </div>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="px-4 py-6 space-y-6 max-w-lg mx-auto">
          ...
          <div className="grid grid-cols-4 gap-3">
            {existingImages.map((img, index) => (
              <div key={`existing-${index}`} className="relative aspect-square rounded-xl overflow-hidden bg-muted">
                ...
                <button type="button" onClick={() => removeExistingImage(index)} className="absolute top-1 right-1 w-6 h-6 bg-destructive text-destructive-foreground rounded-full flex items-center justify-center">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ))}
            {images.map((file, index) => (
              <div key={`new-${index}`} className="relative aspect-square rounded-xl overflow-hidden bg-muted">
                <img src={URL.createObjectURL(file)} alt={`Bilde ${index + 1}`} className="w-full h-full object-cover" />
                <button type="button" onClick={() => removeNewImage(index)} className="absolute top-1 right-1 w-6 h-6 bg-destructive text-destructive-foreground rounded-full flex items-center justify-center">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ))}
        ...
        <div className="space-y-2">
          <Label className="text-base font-semibold">Tittel</Label>
          <Input placeholder="f.eks. Stor pose med flasker" value={title} onChange={(e) => setTitle(e.target.value)} className="h-12" />
        </div>

        <div className="space-y-2">
          <Label className="text-base font-semibold">Beskrivelse</Label>
          <Textarea placeholder="Beskriv hva du har å tilby..." value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className="resize-none" />
        </div>

        <div className="space-y-2">
          <Label className="text-base font-semibold">Antall flasker (valgfritt)</Label>
          <Input type="number" placeholder="0" value={bottleCount} onChange={(e) => setBottleCount(e.target.value)} className="h-12" />
        </div>

        <div className="space-y-2">
          <Label className="text-base font-semibold">Pris (valgfritt)</Label>
          ...
          <p className="text-xs text-muted-foreground">La stå tom eller sett til 0 for donasjon</p>
        </div>

        <div className="space-y-2">
          <Label className="text-base font-semibold">Lokasjon</Label>
          <div className="relative">
            <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
            <Input placeholder="f.eks. Grünerløkka, Oslo" value={location} onChange={(e) => setLocation(e.target.value)} className="h-12 pl-12" />
          </div>
        </div>

        <Button type="submit" variant="hero" size="xl" className="w-full" disabled={isSubmitting}>
          {isSubmitting ? "Publiserer..." : editId ? "Oppdater annonse" : "Publiser annonse"}
        </Button>
      </form>
    </div>
  );
