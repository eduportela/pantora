  ...
  if (!listing) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        <p className="text-muted-foreground">Annonsen ble ikke funnet</p>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <Badge className={listing.type === "donate" ? "bg-success text-success-foreground" : "bg-primary text-primary-foreground"}>
                {listing.type === "donate" ? <><Heart className="w-3 h-3 mr-1" />Donasjon</> : <><Package className="w-3 h-3 mr-1" />Til salgs</>}
              </Badge>
              <h2 className="text-2xl font-bold text-foreground mt-2">{listing.title}</h2>
            </div>
            {listing.type === "sell" && listing.price ? (
              <p className="text-2xl font-bold text-primary">{listing.price} kr</p>
            ) : (
              <p className="text-2xl font-bold text-success">Gratis</p>
            )}
          </div>
          ...
          <div className="bg-card rounded-xl border border-border p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-semibold text-foreground">{profile?.display_name || "Anonym"}</p>
                <p className="text-xs text-muted-foreground">
                  Lagt ut {formatDistanceToNow(new Date(listing.created_at), { addSuffix: true, locale: nb })}
                </p>
              </div>
            </div>
            {/* Contact info - only if public */}
            <div className="flex gap-2 mt-3">
              {profile?.phone_public && profile?.phone && (
                <Button size="sm" variant="outline" className="flex-1" onClick={() => window.location.href = `tel:${profile.phone}`}>
                  <Phone className="w-4 h-4 mr-1" />
                  Ring
                </Button>
              )}
              {profile?.email_public && profile?.email && (
                <Button size="sm" variant="outline" className="flex-1" onClick={() => window.location.href = `mailto:${profile.email}`}>
